...
Sprite::Sprite() :
    ...
    m_currentAnimState("")
{
    SetPosition(10.0f, 60.0f);
}

void Sprite::SetPosition(float x_p, float y_p)
{
    m_posX = x_p;
    m_posY = y_p;
}

void Sprite::SetupCollision()
{
    this->m_collision.box.coord.x = this->GetPosX();
    this->m_collision.box.coord.y = this->GetPosY();
    this->m_collision.box.width = this->m_frameWidth + 80;
    this->m_collision.box.height = this->m_frameHeight + 80;
    this->m_collision.isGrounded = false;
    this->m_collision.isActive = true;
    this->m_collision.isSolid = true;
    this->m_collision.isVisible = true;
}

bool Sprite::CheckCollision(const CollisionObject& objOne, const CollisionObject& objTwo)
{
    SDL_FRect objOne_t = { objOne.box.coord.x, objOne.box.coord.y, objOne.box.width, objOne.box.height };
    SDL_FRect objTwo_t = { objTwo.box.coord.x, objTwo.box.coord.y, objTwo.box.width, objTwo.box.height };
    SDL_FRect intersection_t;
    return SDL_IntersectFRect(&objOne_t, &objTwo_t, &intersection_t);
}

CollisionObject Sprite::GetCollisionBox() const
{
    return this->m_collision;
}

void Sprite::FlipSpriteLeft(bool flip_p)
{
    m_flipHorizontal = flip_p;
}

void Sprite::UpdatePosition(float deltaTime, float velocityX, float velocityY)
{
    // Update sprite's position based on velocity and delta time
    m_posX += velocityX * deltaTime;
    m_posY += velocityY * deltaTime;

    if (this->m_collision.isActive)
    {
        this->m_collision.box.coord.x = m_posX;
        this->m_collision.box.coord.y = m_posY;
    }
}
...
void Sprite::SetAnimationState(const std::string& state, int frameWidth, int frameHeight)
{
    if (m_currentAnimState != state) // Only change if the state is different
    {
        m_currentAnimState = state; // Update the current animation state
        m_frameCounter = 0;
        SetAnim(state, frameWidth, frameHeight); // Call SetAnim to load the new animation
    }
}
...
void Sprite::SetAnim(const std::string& animName_p, int frameWidth_p, int frameHeight_p)
{
    ...
    this->m_frameWidth = frameWidth_p;
    this->m_frameHeight = frameHeight_p;
    this->m_frameTime = 100; // low value == fast anim, high value == slow anim
...
void Sprite::SetImage(ResourceManager& resourceManager, IMAGE_E assetID)
{
...
void Sprite::Render(SDL_Renderer* renderer, float x, float y)
{
    ...
    // Calculate the srcRect based on the current frame
    SDL_Rect srcRect;
    ...
    if (this->m_collision.isVisible)
    {
        SDL_SetRenderDrawColor(renderer, this->m_collision.colour.red, 
            this->m_collision.colour.green, 
            this->m_collision.colour.blue, 
            this->m_collision.colour.alpha);

        SDL_Rect box_t = { (int)m_collision.box.coord.x,
        (int)m_collision.box.coord.y,
        (int)m_collision.box.width ,
        (int)m_collision.box.height };

        SDL_RenderFillRect(renderer, &box_t);
    }

    

    // Determine the flip mode
    SDL_RendererFlip flip = m_flipHorizontal ? SDL_FLIP_HORIZONTAL : SDL_FLIP_NONE;

    // Render the current frame with flipping
    SDL_RenderCopyEx(renderer, m_image.GetTexture(), &srcRect, &dstRect, 0.0, nullptr, flip);
}
